...
pushd "%~dp0%WCROOT%" && (
  ...
  (
    git remote get-url origin > nul 2> nul && call :CMD git remote set-url origin "%%CONTOOLS_ROOT.GIT.ORIGIN%%"
  ) || call :CMD git remote add origin "%%CONTOOLS_ROOT.GIT.ORIGIN%%" || ( popd & goto EXIT )
  (
    git remote get-url tools > nul 2> nul && call :CMD git remote set-url tools "%%CONTOOLS_TOOLS.GIT.ORIGIN%%"
  ) || call :CMD git remote add tools "%%CONTOOLS_TOOLS.GIT.ORIGIN%%" || ( popd & goto EXIT )
  (
    git remote get-url tools-svn > nul 2> nul && call :CMD git remote set-url tools-svn "%%CONTOOLS_TOOLS_SVN.GIT.ORIGIN%%"
  ) || call :CMD git remote add tools-svn "%%CONTOOLS_TOOLS_SVN.GIT.ORIGIN%%" || ( popd & goto EXIT )
