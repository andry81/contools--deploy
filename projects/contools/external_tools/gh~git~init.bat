...
pushd "%~dp0%WCROOT%" && (
  ...
  (
    git remote get-url origin > nul 2> nul && call :CMD git remote set-url origin "%%EXTERNAL_TOOLS.GIT.ORIGIN%%"
  ) || call :CMD git remote add origin "%%EXTERNAL_TOOLS.GIT.ORIGIN%%" || ( popd & goto EXIT )
