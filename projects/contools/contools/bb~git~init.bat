...
pushd "%~dp0%WCROOT%" && (
  rem reinit git svn
  call :GIT_SVN_INIT "%%CONTOOLS_ROOT.SVN.REPOROOT%%" --stdlayout || ( popd & goto EXIT )

  call :CMD git config user.name "%%GIT2.USER%%" || ( popd & goto EXIT )
  call :CMD git config user.email "%%GIT2.EMAIL%%" || ( popd & goto EXIT )

  (
    git remote get-url origin > nul 2> nul && call :CMD git remote set-url origin "%%CONTOOLS_ROOT.GIT2.ORIGIN%%"
  ) || call :CMD git remote add origin "%%CONTOOLS_ROOT.GIT2.ORIGIN%%" || ( popd & goto EXIT )
  (
    git remote get-url tools > nul 2> nul && call :CMD git remote set-url tools "%%CONTOOLS_TOOLS.GIT2.ORIGIN%%"
  ) || call :CMD git remote add tools "%%CONTOOLS_TOOLS.GIT2.ORIGIN%%" || ( popd & goto EXIT )
  (
    git remote get-url tools-svn > nul 2> nul && call :CMD git remote set-url tools-svn "%%CONTOOLS_TOOLS_SVN.GIT2.ORIGIN%%"
  ) || call :CMD git remote add tools-svn "%%CONTOOLS_TOOLS_SVN.GIT2.ORIGIN%%" || ( popd & goto EXIT )
