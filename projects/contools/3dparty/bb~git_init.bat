...
pushd "%~dp0%WCROOT%" && (
  rem reinit git svn
  call :GIT_SVN_INIT "%%CONTOOLS_3DPARTY.SVN.REPOROOT%%" || ( popd & goto EXIT )

  call :CMD git config user.name "%%GIT2.USER%%" || ( popd & goto EXIT )
  call :CMD git config user.email "%%GIT2.EMAIL%%" || ( popd & goto EXIT )

  (
    git remote get-url origin > nul 2> nul && call :CMD git remote set-url origin "%%CONTOOLS_3DPARTY.GIT2.ORIGIN%%"
  ) || call :CMD git remote add origin "%%CONTOOLS_3DPARTY.GIT2.ORIGIN%%" || ( popd & goto EXIT )
