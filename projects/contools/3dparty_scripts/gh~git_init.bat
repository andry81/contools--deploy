...
pushd "%~dp0%WCROOT%" && (
  ...
  (
    git remote get-url origin > nul 2> nul && call :CMD git remote set-url origin "%%CONTOOLS_3DPARTY_SCRIPTS.GIT.ORIGIN%%"
  ) || call :CMD git remote add origin "%%CONTOOLS_3DPARTY_SCRIPTS.GIT.ORIGIN%%" || ( popd & goto EXIT )
