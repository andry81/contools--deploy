...
pushd "%~dp0%WCROOT%" && (
  ...
  (
    git remote get-url origin > nul 2> nul && call :CMD git remote set-url origin "%%CONTOOLS_BITTOOLS.GIT3.ORIGIN%%"
  ) || call :CMD git remote add origin "%%CONTOOLS_BITTOOLS.GIT3.ORIGIN%%" || ( popd & goto EXIT )
