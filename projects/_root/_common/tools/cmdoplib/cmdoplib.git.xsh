    ...
    with GitReposListReader(configure_dir + '/git_repos.lst') as repos_reader:
      ...
      for row in repos_reader:
        if row['scm_token'] == scm_name and row['branch_type'] != 'root':
          ...
          svn_path_prefix_escaped = git_regex_escape_svn_path_prefix(yaml_expand_value(svn_path_prefix))
...
def git_pull(configure_dir, scm_name):
  ...
  with local.cwd(wcroot_path), GitReposListReader(configure_dir + '/git_repos.lst') as repos_reader:
    has_root = False
    for row in repos_reader:
      # pull the root only
      if row['scm_token'] == scm_name and row['branch_type'] == 'root':
        ...
        if local_branch == '.' and remote_branch == '.':
          raise Exception('at least one parameter must cointain a valid git branch name: local=`{0}` remote=`{1}`'.format(local_branch, remote_branch))

        pull_branch_token = local_branch
        if pull_branch_token == '.': pull_branch_token = ''
        if pull_branch_token == '':
          pull_branch_token = remote_branch
        elif remote_branch != '' and remote_branch != '.' and remote_branch != local_branch:
          pull_branch_token = remote_branch + ':' + pull_branch_token

        call('git', ['pull', remote_name, pull_branch_token])

        break

    if not has_root:
      Exception('Hava has no root branch in the git_repos.lst')

    """
    # subtree pull (invalid implementation, must be done through the pure `git fetch` w/o check out)
    for row in repos_reader:
      if row['scm_token'] == scm_name and row['branch_type'] == 'subtree':
        remote_name = yaml_expand_value(row['remote_name'])
        remote_url = yaml_expand_value(row['remote_url'])
        local_branch = yaml_expand_value(row['local_branch'])
        remote_branch = yaml_expand_value(row['remote_branch'])
        remote_branch = yaml_expand_value(row['remote_branch'])
        git_path_prefix = row['git_path_prefix']
        git_subtree_cmdline = row['git_subtree_cmdline']
        ...
        # expand if contains a variable substitution
        git_path_prefix = yaml_expand_value(git_path_prefix)

        if git_subtree_cmdline == '.': git_subtree_cmdline = ''

        git_subtree_cmdline_list = shlex.split(git_subtree_cmdline)

        ret = call_no_except('git', ['subtree', 'add', '--prefix=' + git_path_prefix] + git_subtree_cmdline_list + [remote_name, pull_branch_token])
        if ret[0]:
          call('git', ['subtree', 'pull', '--prefix=' + git_path_prefix] + git_subtree_cmdline_list + [remote_name, pull_branch_token])
    """
