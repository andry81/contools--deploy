...
pushd "%~dp0%WCROOT%" && (
  rem reinit git svn
  call :GIT_SVN_INIT "%%_3DPARTY_SCRIPTS.SVN.REPOROOT%%" || ( popd & goto EXIT )

  call :CMD git config user.name "%%GIT3.USER%%" || ( popd & goto EXIT )
  call :CMD git config user.email "%%GIT3.EMAIL%%" || ( popd & goto EXIT )

  (
    git remote get-url origin > nul 2> nul && call :CMD git remote set-url origin "%%_3DPARTY_SCRIPTS.GIT3.ORIGIN%%"
  ) || call :CMD git remote add origin "%%_3DPARTY_SCRIPTS.GIT3.ORIGIN%%" || ( popd & goto EXIT )
